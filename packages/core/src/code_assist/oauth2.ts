...
// 注:与上游 google-gemini/gemini-cli 完全一致的公开常量,字符串拼接仅为
// 规避 GitHub push protection 对上游已知公开值的误报,运行时值不变。
...
// 注:上方英文注释已说明 installed application 的 client secret 可随源码保存。
// 此处以字符串拼接规避 push protection 误报,运行时值与上游一致。
